...
def structure_of_10k(text):
    ...
    for i ,(item, start_pos) in enumerate(matches):
        ...
        new_item, identifier = create_part(i, start_pos, matches, text)
        part.add_item(identifier, new_item)
    
    

def create_part( i, start_pos, matches, text, item_id=None):
...
def convert_10k(path ):
    with open(path, "r") as f:
        text = f.read()

    structure_of_10k(text)
    return report
