from models import Report8K, Section, Item
import re

def structure_of_8k(text,report):
        ...
        if item:
            ...
            if match:
                ...
                if section_number in report.sections:
                    new_item = create_item(item_number,item_title,i, start_pos, matches, text)
                    # print(type(section))
                    section.add_item(item_number, new_item)



def create_item(item_number, item_title, i:int, start_pos, matches, text):
...
def data_8k(report,path ):
    with open(path, "r") as f:
        text = f.read()

# report = Report8K()
    structure_of_8k(text, report)
    return report
